 ...
 */
Plugin.prototype.createReadStream = function(str, options, callback) {
  // rough check for a scheme with an authority
  if (!str.match(/^[^:]+:\/\//)) {
    // assume file path
    var stream = fs.createReadStream(str, options);
    if (old) {
    ...
    if (protocol) {
      protocol.request(str, options, function(stream) {
        if (old) {
          stream = new stream.Readable().wrap(stream);
        }
        callback.call(null, stream);
      ...
      callback.call(new Error('Unsupported protocol: ' + parts.protocol));
    }
  }
};
